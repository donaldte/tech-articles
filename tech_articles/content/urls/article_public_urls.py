...
urlpatterns = [
    ...
    path("articles/", ArticlesListView.as_view(), name="home_articles_list"),
    path(
        "articles/<slug:slug>/", ArticleDetailView.as_view(), name="home_article_detail"
    ),
    # Article listing and data APIs
    path("api/articles/", ArticlesApiView.as_view(), name="api_articles"),
    path(
        "api/articles/featured/",
        FeaturedArticlesApiView.as_view(),
        name="api_featured_articles",
    ),
    path(
        "api/articles/related/",
        RelatedArticlesApiView.as_view(),
        name="api_related_articles",
    ),
    path("api/categories/", CategoriesApiView.as_view(), name="api_categories"),
    # Interactive APIs for articles
    path(
        "api/articles/<uuid:article_id>/clap/",
        ArticleClapApiView.as_view(),
        name="api_article_clap",
    ),
    path(
        "api/articles/<uuid:article_id>/like/",
        ArticleLikeApiView.as_view(),
        name="api_article_like",
    ),
    path(
        "api/articles/<uuid:article_id>/comments/",
        ArticleCommentApiView.as_view(),
        name="api_article_comments",
    ),
